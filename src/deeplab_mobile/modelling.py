...
def select_images_input(images_list, type_names=TYPE_NAMES, type_to_use="flair"):
    images_chosen = np.array(
        [
            images_list[type_names.index(type_to_use)][i]
            for i in range(len(images_list[type_names.index(type_to_use)]))
        ]
    )
    images_seg = np.array(
        [
            images_list[type_names.index("seg")][i]
            for i in range(len(images_list[type_names.index("seg")]))
        ]
    )
...
def train_deeplab_mobile(
    ...
    criterion=torch.nn.MSELoss(
        reduction="mean"
    ),  # TODO: torch.nn.CrossEntropyLoss(reduction='mean') check activation expected / binary crosssentropy
    num_epochs=2,
):
